...
export async function GET(request: NextRequest) {
  console.log('Testing Google Calendar Composio integration');
  
  try {
    // Extract parameters
    const { searchParams } = new URL(request.url);
    const merchantId = searchParams.get('merchantId');
    const customIntegrationId = searchParams.get('integrationId') || 'e4c1c614-421c-4ab2-9544-20b3b1b8c5d3';
    ...
    try {
      await setDoc(
        doc(db, 'merchants', merchantId, 'integrations', 'google_calendar'),
        {
