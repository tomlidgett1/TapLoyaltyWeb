...
export async function GET(request: NextRequest) {
  console.log('Testing Google Docs Composio integration');
  
  try {
    // Extract parameters
    const { searchParams } = new URL(request.url);
    const merchantId = searchParams.get('merchantId');
    const customIntegrationId = searchParams.get('integrationId') || 'b75caef3-ef36-4abd-893a-a350cc1d4a31';
    ...
    console.error('Error in Google Docs integration:', error);
