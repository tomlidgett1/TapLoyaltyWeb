...
export async function GET(request: NextRequest) {
  console.log('Google Sheets Composio callback received');
  ...
  try {
    ...
    // Now we need to check all existing Google Sheets integrations and update their status
    if (code) {
      try {
        console.log('OAuth successful, checking all Google Sheets integrations for updates...');
        ...
        console.log(`Found ${allConns.length} connected accounts for Google Sheets integration`);
          ...
          if (account.status === 'ACTIVE' && account.entityId) {
            ...
            try {
              ...
              if (merchantDoc.exists()) {
                // Update the Google Sheets integration status
                await updateDoc(
                ...
                console.log(`Successfully updated Google Sheets integration for merchant: ${merchantId}`);
              } else {
        ...
        console.log('Finished updating all active Google Sheets connections');
        ...
        if (state) {
          try {
            ...
            if (merchantDoc.exists()) {
              ...
              if (merchantAccount) {
                await updateDoc(
                  doc(db, 'merchants', state, 'integrations', 'google_sheets'),
                  {
                    connected: true,
                ...
                console.log(`Successfully updated Google Sheets integration via state parameter for merchant: ${state}`);
              }
            }
          } catch (stateError) {
    ...
    return NextResponse.redirect('/dashboard/integrations?success=true&source=composio_google_sheets_callback');
  } catch (error) {
    console.error('Error handling Google Sheets Composio callback:', error);
