...
export function DemoIPhone({ open, onOpenChange, isDemoMode = false, initialTab = 'rewards', previewData }: DemoIPhoneProps) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    const fetchMerchantData = async () => {
      ...
      if (previewData) {
        ...
        setMerchantName(previewData.merchantName || "Your Store")
        setDisplayAddress("123 Main St, Melbourne VIC 3000")
        
        // Set coffee program from preview data
        if (previewData.coffeePrograms && previewData.coffeePrograms.length > 0) {
          const firstCoffeeProgram = previewData.coffeePrograms[0]
          setCoffeeProgram({
            active: true,
            frequency: firstCoffeeProgram.frequency || 10,
            description: `Buy ${(firstCoffeeProgram.frequency || 10) - 1} coffees, get 1 free!`
          })
        }
        
        // Set tap cash from preview data
        if (previewData.tapCash && previewData.tapCash.enabled) {
          setCashbackProgram({
            isActive: true,
            description: previewData.tapCash.description,
            cashbackRate: previewData.tapCash.rate
          })
        }
        
        // Convert preview rewards to display format
        if (previewData.rewards) {
          const convertedRewards = previewData.rewards.map((reward, index) => ({
            id: `preview_${index}`,
            rewardName: reward.name || 'Reward',
            description: reward.description || `Get ${reward.discountAmount}${reward.discountType === 'percentage' ? '%' : '$'} off`,
            type: 'discount',
            programType: '',
            pointsCost: reward.pointsRequired || 0,
            voucherAmount: 0,
        ...
        // Convert voucher programs to rewards
        if (previewData.voucherPrograms) {
          const voucherRewards = previewData.voucherPrograms.map((voucher, index) => ({
            id: `voucher_${index}`,
            rewardName: voucher.rewardName || 'Voucher Program',
            description: `Spend $${voucher.spendRequired} to earn $${voucher.discountAmount} off`,
            type: 'voucher',
            programType: 'voucher',
            pointsCost: 0,
          ...
          setRewards(prev => [...prev, ...voucherRewards])
        }
  ...
  // If onOpenChange is a no-op function, we're in preview mode
  const isPreviewMode = onOpenChange.toString().includes('{}')

  if (isPreviewMode) {
    // Return the iPhone without backdrop or animation for preview mode
    return (
