...
export function NetworkRewardPopup({ open, onOpenChange }: NetworkRewardPopupProps) {
  ...
  const [isNetworkStore, setIsNetworkStore] = useState<boolean | null>(null)
  const [isCheckingNetworkStatus, setIsCheckingNetworkStatus] = useState(false)
  ...
  // Listen for real-time network store status updates
  useEffect(() => {
    if (!user) {
      setIsNetworkStore(false)
      setIsCheckingNetworkStatus(false)
      return
    }

    // Only show checking state on initial load if we don't have the data
    if (isNetworkStore === null) {
      setIsCheckingNetworkStatus(true)
    }

    const merchantRef = doc(db, 'merchants', user.uid)
    const unsubscribe = onSnapshot(merchantRef, (docSnapshot) => {
      if (docSnapshot.exists()) {
        const merchantData = docSnapshot.data()
        setIsNetworkStore(merchantData.isNetworkStore === true)
      } else {
        setIsNetworkStore(false)
      }
      setIsCheckingNetworkStatus(false)
    }, (error: any) => {
      console.error("Error listening to network store status:", error)
      setIsNetworkStore(false)
      setIsCheckingNetworkStatus(false)
    })
  ...
  const handleStepChange = (step: number) => {
    // Check if user is allowed to create network rewards
    if (step > 1 && isNetworkStore === false) {
      toast({
        title: "Account not eligible",
        description: "Network rewards are only available to Network Store accounts. Please contact support to upgrade your account.",
        variant: "destructive"
      })
      return
    }
    ...
    <>
      <Dialog 
        ...
        onOpenChange={(open) => {
          ...
          // Don't reset network store status - keep it cached
        }
          onOpenChange(open)
        }}
      >
              ...
              <div className="flex-1 flex flex-col h-full">
                <div className="flex-1 overflow-y-auto p-6 min-h-0">
                  {currentStep === 1 && (
                    <div className={cn(
                      ...
                      infoBoxesVisible ? "mt-0 space-y-3" : "-mt-4 space-y-3"
                    )}>
                      {/* Network Store Requirement Message */}
                      {!isCheckingNetworkStatus && isNetworkStore === false && (
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <h4 className="text-sm font-medium text-gray-800 mb-1">
                                Network Store Account Required
                              </h4>
                              <p className="text-sm text-gray-700">
                                Network rewards are only available to merchants with Network Store accounts. 
                                Contact support to upgrade your account and access cross-network customer acquisition features.
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      <div 
                        className={cn(
                          "relative overflow-hidden transition-all duration-500 ease-in-out pt-3",
                        ...
                        <div className="relative">
                          <div className="overflow-hidden rounded-xl">
                            ...
                            >
                              ...
                              <div className="bg-white border border-gray-200 rounded-xl p-3 text-xs text-gray-700 w-full flex-shrink-0">
                ...
                <div className="border-t border-gray-200 p-4 md:p-6 bg-gray-50 flex-shrink-0">
                  <div className="flex flex-col sm:flex-row justify-between gap-3 max-w-5xl">
                    ...
                    <Button 
                      onClick={() => {
                        if (currentStep === 1 && isNetworkStore === false) {
                      ...
                      disabled={isSubmitting || isCheckingNetworkStatus}
                      className="bg-[#007AFF] hover:bg-[#0062CC] text-white rounded-md w-full sm:w-auto order-1 sm:order-2 disabled:bg-gray-400 disabled:cursor-not-allowed"
                    >
                      {isCheckingNetworkStatus ? 'Checking Account...' : 
                       currentStep === 1 && isNetworkStore === false ? 'Switch to Network Store' :
                       currentStep === 2 ? (isSubmitting ? 'Creating Reward...' : 'Create Network Reward') : 'Continue'}
                      {currentStep === 1 && isNetworkStore === true && <ArrowRight className="ml-1 h-3 w-3" />}
                    </Button>
                  </div>
                </div>
              </div>
